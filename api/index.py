        ...
        try:
            ...
            if client_id and client_secret and refresh_token:
                try:
                    ...
                    print(f"Error fetching live AdMob data: {e}")

            # Lấy dữ liệu Google Sheets cho Quicksave & LunaAI
            try:
            ...
            print(f"Error in do_GET realtime processing block: {e}")

        # Sắp xếp lại log theo ngày giảm dần chuẩn format Chart
        sorted_history = dict(sorted(history_data.items(), reverse=True))
        body = json.dumps(sorted_history).encode('utf-8')

        # Cấu hình Headers (CORS và Catching) ở cuối cùng
